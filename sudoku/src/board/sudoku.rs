...
impl Sudoku {
    ...
    /// Generate a random, sudoku, solvable with the given strategies
    /// The puzzles are minimal in that no cell can be removed without losing uniquess of solution
    pub fn generate_unique_strat(strategies: &[Strategy]) -> Self {
        ...
        for &cell in &cell_order[0..] {
            let mut sudoku_tmp = sudoku;
            sudoku_tmp.0[cell] = 0;

            let solver = StrategySolver::from_sudoku(sudoku_tmp);
    ...
    /// Generate a random, sudoku, solvable with the given strategies
    /// The puzzles are minimal in that no cell can be removed without losing uniquess of solution
    pub fn generate_max_missing(miss: i32) -> Self {
        let mut sudoku = Sudoku::generate_filled();
